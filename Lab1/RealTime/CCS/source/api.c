...
void PWM_Pram(){
    ...
    switch(sel_pwm){
        case 0:
            freq = 4;
            dutyCycle = 0.75;
            period_time = 92.0/freq - 5;
            ton =  dutyCycle * (period_time + 5) + 1;
            break;
        case 1:
            freq = 2;
            dutyCycle = 0.25;
...
//---------------------------------------------------------------------
//           Real Time Task - Print String to LED
//---------------------------------------------------------------------
void PrintStr(char str[] , unsigned int delay_ms){
    unsigned int i = 0;

    while (str[i] != '\0'){
        print2LEDs(str[i]); // Print char to LED
